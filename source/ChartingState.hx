	...
	**/
	...
	function deleteNote(note:Note):Void
	{
		...
		for (n in 0..._song.notes[curSection].sectionNotes.length)
		{
			...
			if ((i[0] == note.strumTime + FlxG.save.data.offset + (note.strumTime == 0 ? 0 : 1)
				? true : i[0] == note.strumTime+FlxG.save.data.offset)
				&& i[1] % 4 == note.noteData)
				// Why does it do this?
				// I DONT FUCKING KNOW!!!!!!!!!!!!!!
			{
	...
	{
		PlayState.SONG = Song.loadFromJson(song.toLowerCase(), song.toLowerCase()=='you-are-a-fool' ? "dontgopeekingaroundwhereyoushouldnt" : song.toLowerCase());
		FlxG.resetState();
	}
