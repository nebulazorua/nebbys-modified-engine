...
class PlayState extends MusicBeatState
{
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.save.data.accuracyDisplay)
		{
			scoreTxt.text = "Score:" + songScore + " | Misses:" + misses + " | Accuracy:" + truncateFloat(accuracy, 2) + "% | " + grade;
		}
		else
		{
	...
	function endSong():Void
	{
		...
		{
			#if !switch
			trace("SAVIN DA SCORE " + SONG.song + storyDifficulty + " SCORE: " + songScore);
	...
	private function popUpScore(strumtime:Float):Void
		{
			...
			if(FlxG.save.data.inputSystem=="New KadeEngine"){
				...
				if (noteDiff > Conductor.safeZoneOffset * 2 || noteDiff < Conductor.safeZoneOffset * -2)
					...
					daRating= "bad";
				else if (noteDiff > Conductor.safeZoneOffset * 0.25 || noteDiff < Conductor.safeZoneOffset * -0.25)
					daRating = "good";
				else
					daRating = "sick";
			}
	...
	function updateAccuracy()
		{
			trace("update accuracy " + (totalPlayed+1) + " / " + totalNotesHit);
			fc = misses>0?false:true;

			totalPlayed += 1;
			accuracy = totalNotesHit / totalPlayed * 100;
			if(fc){
				grade= accuracy==100?"100% FC":"FC";
			}else{
				var bruh:Array<Bool>=[
					accuracy>=95, // S
					accuracy>=90, // A+
	...
	function vanillaNoteCheck(keyP:Bool, note:Note):Void
	{
		trace("KEYP " + keyP, note.noteData);
		if (keyP)
			goodNoteHit(note);
		else
		{
	...
	{
		if(FlxG.save.data.inputSystem=="Vanilla"){
			vanillaNoteCheck(controlArray[note.noteData],note);
		}else{
			kadeNoteCheck(controlArray,note);
		}
	}
	function kadeNoteCheck(controlArray:Array<Bool>, note:Note):Void // sorry lol
		{
			if (loadRep)
			{
